...
mean_sd <- function(dist = "normal", par = NULL) {
  ...
  if(dist %in% c("exp", "exponential")) if(!is.null(par$rate) && par$rate <=0) dist <- ""
  switch(dist,
         ...
         {
           qf <- tryCatch(get_qfun(dist), error = function(e) NULL)
           if(is.null(qf)) return(list(mean = NA, sd = NA))
           par_pos <- unname(as.numeric(par))
           mu_est <- tryCatch(
             stats::integrate(f = function(u) do.call(qf, c(list(u), par_pos)),
                              lower = .Machine$double.eps,
                              upper = 1 - .Machine$double.eps)$value,
             error = function(e) NA_real_)
           sd_est <- tryCatch({
             mu2 <- stats::integrate(f = function(u) do.call(qf, c(list(u), par_pos))^2,
                                     lower = .Machine$double.eps,
                                     upper = 1 - .Machine$double.eps)$value
             sqrt(max(0, mu2 - mu_est^2))
           }, error = function(e) NA_real_)
           list(mean = mu_est, sd = sd_est)
         }
  )
}
...
dist_moments <- function(dist = "normal", pars = NULL, included = c("mean", "var", "sd", "median", "mode"), quantiles = c("2.5%" = 0.025, "5%" = 0.05, "25%" = 0.25, "50%" = 0.5, "75%" = 0.75, "95%" = 0.95, "97.5%" = 0.975)) {
  ...
  thismode <- switch(dist,
                     ...
                     weibull = ifelse(test = pars$par1 > 1,
                                      ...
                                      no = 0),
                     {
                       local_pars <- unname(pars)
                       do.call(thisq, c(
                         list(optim(par = c(thisval = 0.5),
                                    lower = c(thisval = 0),
                                    upper = c(thisval = 1),
                                    fn = function(thisval) {
                                      do.call(thisd, c(list(do.call(thisq, c(list(thisval), local_pars))),
                                                       local_pars))
                                    },
                                    method = 'L-BFGS-B',
                                    control = list(fnscale = -1))[[1]][[1]]),
                         local_pars))
                     }
  )
  ...
  tmp <- c(switch(dist,
                  ...
                  {
                    local_pars <- unname(pars)
                    qf <- tryCatch(get_qfun(dist), error = function(e) thisq)
                    mu_est <- tryCatch(
                      stats::integrate(f = function(u) do.call(qf, c(list(u), local_pars)),
                                       lower = .Machine$double.eps,
                                       upper = 1 - .Machine$double.eps)$value,
                      error = function(e) NA_real_)
                    var_est <- tryCatch({
                      mu2 <- stats::integrate(f = function(u) do.call(qf, c(list(u), local_pars))^2,
                                              lower = .Machine$double.eps,
                                              upper = 1 - .Machine$double.eps)$value
                      max(0, mu2 - mu_est^2)
                    }, error = function(e) NA_real_)
                    c(mean   = mu_est,
                      var    = var_est,
                      sd     = sqrt(var_est),
                      median = tryCatch(do.call(qf, c(list(.5), local_pars)),
                                        error = function(e) NA_real_))
                  }),
           mode = thismode,
